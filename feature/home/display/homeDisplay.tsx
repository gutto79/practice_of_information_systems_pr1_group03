...
const HomeDisplay = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (coupleData) {
        ...
        // 确定配对用户的ID
        const partnerUid =
          coupleData.uid1 === user.id ? coupleData.uid2 : coupleData.uid1;
      ...
      if (actionsError) {
        ...
        console.log(
          "【数据详情】第一条记录示例:",
          JSON.stringify(recentActionsData[0], null, 2)
        );
        ...
        const formattedActions = recentActionsData
          .filter((record) => {
            console.log("【数据检查】检查记录:", {
              ...
              actionLength: Array.isArray(record?.Action)
                ? record.Action.length
                : 1,
              fullRecord: record,
            });
  ...
  const handleBreakup = async () => {
    ...
    await supabase
      ...
      .delete()
      .or(
        `and(uid1.eq.${user.id},uid2.eq.${partnerId}),and(uid1.eq.${partnerId},uid2.eq.${user.id})`
      );
              ...
              <div key={invite.id} className="flex items-center gap-2 mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1 bg-gray-300 text-gray-700 rounded whitespace-nowrap"
                  >
                    拒否
                  </button>
                </div>
  ...
  return (
    <div className="relative p-6 h-screen">
        ...
        <div className="w-full max-w-xs">
          ...
          <div
            className={`relative w-full h-5 ${getBarColor(
              partnerGender,
              "bg"
            )} rounded-full shadow-inner overflow-hidden ${getBorderColor(
              partnerGender
            )}`}
          >
            <div
      ...
      <div className="absolute top-4 right-4 w-24">
        ...
        <div
          className={`relative w-full h-4 ${getBarColor(
            userGender,
            "bg"
          )} rounded-full overflow-hidden ${getBorderColor(userGender)}`}
        >
          <div
            ...
            <button
              onClick={async () => {
                ...
                try {
                  // daysパラメータを設定
                  let days = 1;
                  if (selectedTimeRange === "1週間") {
                    days = 7;
                  } else if (selectedTimeRange === "1ヶ月") {
                    days = 30;
                  }
                  ...
                  if (!response.ok) {
                    ...
                    throw new Error(
                      responseData.error || "動画生成に失敗しました"
                    );
                  }

                  // 開発モードの場合は注意書きを表示
                  if (responseData.note) {
                    console.info("開発モード情報:", responseData);
